...
int main (int argc, char *argv[])
{
   ...
   cuda_ret = cudaMalloc((void **) &A_d, sizeof(float)*A_sz);
    ...
    if(cuda_ret != cudaSuccess) printf("Unable to copy from host to device");
    ...
    if(cuda_ret != cudaSuccess) printf("Unable to copy from device to host");
    ...
    if(matArow <= 5000 && matAcol <= 5000 && matBcol <= 5000) {
        printf("Verifying results..."); fflush(stdout);
        verify(A_h, B_h, C_h, matArow, matAcol, matBcol);
    }
    else printf("Matrices too large for CPU verification\n\n");
